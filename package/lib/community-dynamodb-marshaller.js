...
const convertToSetAttr = (set, options) => {
    const setToOperate = set;
    if (setToOperate.has(undefined)) {
        throw new Error(`Pass options.removeUndefinedValues=true to remove undefined values from map/array/set.`);
    }
    ...
    if (item instanceof NumberValue) {
        ...
        return {
            NS: Array.from(setToOperate)
                .map((num) => convertToNumberAttr(num))
                .map((item) => item.N),
        };
    }
...
const convertToMapAttrFromIterable = (data, options) => ({
    M: ((data) => {
        ...
        for (const [key, value] of data) {
            if (typeof value !== "function" && (value !== undefined || true)) {
    ...
    for (const [key, value] of Object.entries(data)) {
        if (value !== undefined) {
            switch (key) {
                ...
                case "NS":
                    return new Set(value.map((item) => convertNumber(item)));
                case "BS":
...
const convertList = (list, options) => list.map((item) => convertToNative(item));
const convertMap = (map, options) => Object.entries(map).reduce((acc, [key, value]) => ((acc[key] = convertToNative(value)), acc), {});
